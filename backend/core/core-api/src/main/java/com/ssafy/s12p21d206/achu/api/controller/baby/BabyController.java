...
public class BabyController {
  ...
  @PostMapping("/babies")
  public ApiResponse<DefaultIdResponse> appendBaby(@RequestBody AppendBabyRequest request) {
  ...
  public ApiResponse<BabyDetailResponse> findBaby(Long userId, @PathVariable Long babyId) {
    ...
    BabyDetailResponse response = new BabyDetailResponse(
        babyId, "강두식", "여자", "https://test-image.png", LocalDate.of(2025, 3, 20));
    return ApiResponse.success(response);
  }
  ...
  public ApiResponse<BabyDetailResponse> modifyProfileImage(
      Long userId, @PathVariable Long babyId, @RequestBody ModifyProfileImageRequest request) {

    LocalDate birth = LocalDate.of(2025, 3, 20);
    BabyDetailResponse response =
        new BabyDetailResponse(babyId, "강두식", "여자", request.img_url(), birth);
    return ApiResponse.success(response);
  }

  @PatchMapping("/babies/{babyId}")
  public ApiResponse<BabyDetailResponse> modifyBaby(
      Long userId, @PathVariable Long babyId, @RequestBody ModifyBabyRequest request) {

    BabyDetailResponse response = new BabyDetailResponse(
        babyId, request.nickname(), request.gender(), "https://test-image.png", request.birth());
    return ApiResponse.success(response);
  }
