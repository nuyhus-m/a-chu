...
public class GoodsImageFacade {
  ...
  public GoodsDetail append(User user, NewGoods newGoods, List<File> imageFiles) {
    ImageUrlsWithThumbnail imageUrlsWithThumbnail = uploadImageUrlsWithThumbnail(imageFiles);
  ...
  private ImageUrlsWithThumbnail uploadImageUrlsWithThumbnail(List<File> imageFiles) {
    String thumbnailImageUrl =
        imageService.uploadImage(imageFiles.get(0), ImageCompressionOption.THUMBNAIL_IMAGE);
    List<String> imageUrls = imageService.uploadImages(imageFiles, ImageCompressionOption.ORIGINAL);
