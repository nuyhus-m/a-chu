...
@Component
public class AuthImageCompressor {
  ...
  public AuthFile compressFile(AuthFile file, AuthImageCompressionOption option) {
  ...
  private boolean isOriginalSettings(AuthImageCompressionOption option) {
    return option.equals(AuthImageCompressionOption.ORIGINAL);
  }

  private byte[] compressImage(byte[] content, AuthImageCompressionOption option) {
  ...
  private AuthFile createCompressedFile(AuthFile originalFile, byte[] compressedBytes) {
    return new AuthFile(
